const x_data = () => {
  return {
    ...
    gpsTracker: {
      ...
      // Kalibrasi arah haluan default img/kapal.png (radian). Ubah nilai ini kalau
      // arah kapal di layar masih belum pas: 0, Math.PI/2, Math.PI, atau -Math.PI/2.
      shipAngleOffset: 0,
    },
    ...
    // Konversi heading kompas (derajat, 0=Utara, searah jarum jam) ke sudut rotasi
    // canvas. Memakai rotasi arena (theta) YANG SAMA dengan xyToCanvas(), supaya arah
    // haluan kapal konsisten dengan arah garis trajectory yang digambar di canvas.
    // Tanpa ini, heading kompas langsung dipetakan ke ctx.rotate() tanpa mengikuti
    // rotasi 245 derajat yang dipakai untuk memutar posisi -> haluan kapal jadi
    // tidak sinkron dengan garis lintasan (bisa kelihatan menghadap/berjalan mundur).
    headingToCanvasAngle(headingDeg) {
      const headingRad = (headingDeg * Math.PI) / 180;
      const theta = (245 * Math.PI) / 180; // HARUS SAMA dengan theta di xyToCanvas()

      // vector arah kompas dalam frame lokal (east, north)
      const dx = Math.sin(headingRad);
      const dy = Math.cos(headingRad);

      // rotasi arena, sama seperti transformasi posisi di xyToCanvas()
      const xr = dx * Math.cos(theta) - dy * Math.sin(theta);
      const yr = dx * Math.sin(theta) + dy * Math.cos(theta);

      // flip Y karena canvasY = originY - yr*scale (sama seperti xyToCanvas())
      let angle = Math.atan2(-yr, xr);

      // offset kalibrasi arah default gambar img/kapal.png (haluan menghadap ke mana
      // saat angle = 0 sebelum dirotasi). Ubah gpsTracker.shipAngleOffset (dalam
      // radian) sampai arah kapal di layar cocok dengan arah gerak aslinya:
      // 0 = tidak ada offset, Math.PI/2 = +90°, Math.PI = 180° (kebalik total),
      // -Math.PI/2 = -90°.
      angle += this.gpsTracker.shipAngleOffset ?? 0;
    ...
    drawOrigin() {
      // PENTING: fungsi ini HANYA menandai titik awal (0,0) tempat tracking dimulai,
      // BUKAN posisi kapal saat ini. Sebelumnya di sini ikut digambar ship image
      // dengan rotasi heading -> muncul "kapal hantu" yang diam terus di titik start
      // sementara kapal asli sudah digambar di drawPath() pada titik terakhir.
      // Dua ikon kapal yang tumpang tindih inilah yang bikin terlihat seperti kapal
      // mundur balik ke titik awal padahal fisiknya maju. Sekarang origin cuma
      // ditandai titik merah kecil, ship SELALU digambar di posisi terakhir (drawPath).
      const ctx = this.gpsTracker.ctx;
    ...
    drawPath() {
      ...
      path.forEach((p, i) => {
        ...
        if (isLast) {
          // Pakai heading kompas asli, ditransformasi dengan rotasi arena (theta)
          // yang sama seperti posisi -> arah kapal konsisten dengan garis lintasan.
          const headingDeg = this.vehicleData.heading ?? 0;
          const angle = this.headingToCanvasAngle(headingDeg);

          // draw ship di posisi TERAKHIR/SAAT INI (satu-satunya ikon kapal)
          this.drawShip(ctx, p.canvasX, p.canvasY, angle);
