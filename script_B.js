const x_data = () => {
  return {
    ...
    xyToCanvas(x, y) {
      const originCanvasX = 130; // titik awal X saat tracking mulai
      const originCanvasY = 470; // titik awal Y saat tracking mulai
    ...
    drawExamplePattern() {
      ...
      ctx.fillStyle = red;
      ctx.fillRect(w * 0.18, h * 0.81, boxWidth, boxHeight);
      ctx.fillStyle = green;
      ctx.fillRect(w * 0.6, bottomY - h * 0.08, boxWidth, boxHeight);

      ctx.fillStyle = blue;
      ctx.fillRect(w * 0.52, bottomY + h * 0.06, boxWidth, boxHeight);
    },
    drawGPSCanvas() {
